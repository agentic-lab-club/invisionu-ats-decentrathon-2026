...
const STAGES: StageConfig[] = [
  {
    ...
    description: 'Your documents are being verified and processed by our team.',
    icon: Search,
  },
  {
    key: 'application_review',
    label: 'Under Review',
    description: 'The admissions committee is reviewing your application in detail.',
    icon: FileText,
  },
  {
    key: 'decision',
    label: 'Decision',
    description: 'The committee has reached a final decision on your application.',
    icon: Gavel,
  },
];
...
function StageStep({
  ...
  const iconStyle =
    status === 'done'
      ? { background: '#b5e220', color: '#1a2e05', border: '2px solid #b5e220' }
      : status === 'active'
      ? {
          background: '#fff',
          color: '#4d7c0f',
          border: '2px solid #b5e220',
          boxShadow: '0 0 0 4px rgba(181,226,32,0.15)',
        }
          ...
          <div
            ...
            style={{
              ...
              background: status === 'done'
                ? 'linear-gradient(to bottom, #b5e220, #d9f99d)'
                : '#e5e7eb',
            }}
          />
        )}
      ...
      <div className="pb-7 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p
            ...
            style={{ color: status === 'pending' ? '#9ca3af' : '#111827' }}
          >
            {stage.label}
          </p>
          ...
          {isLast && status !== 'pending' && reviewStage && (
        ...
        <p
          ...
          style={{ color: status === 'pending' ? '#d1d5db' : '#6b7280' }}
        >
          {stage.description}
        </p>
      </div>
      ...
      <div className="px-5 py-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-white/70 mt-0.5 leading-snug">
            Receive instant updates when your application moves to the next stage
          </p>
        </div>
  ...
  if (stage === 'decision' && decision === 'rejected') {
    return (
      <div className="rounded-2xl p-5" style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
        ...
        <a
          href="mailto:admissions@invisionu.edu"
      ...
      {[
        {
          ...
          body: 'Most applications receive a decision within 2–3 weeks of submission.',
          color: '#6b7280',
          bg: '#f9fafb',
          border: '#e5e7eb',
        },
        {
          icon: Phone,
          title: 'Need help?',
          body: 'Reach our admissions team at admissions@invisionu.edu with any questions.',
          color: '#4d7c0f',
          bg: '#f7fde8',
          border: '#d9f99d',
        },
...
function EmptyState({ onApply }: { onApply: () => void }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-10 flex flex-col items-center text-center gap-4">
      ...
      <button
        onClick={onApply}
        className="mt-2 px-5 py-2.5 text-sm font-semibold rounded-xl transition-colors"
        style={{ background: '#b5e220', color: '#1a2e05' }}
      >
        Start application
      </button>
...
export default function StatusPage() {
  ...
  // Загрузка при монтировании + автообновление
  useEffect(() => {
    fetchStatus();

    const handleFocus = () => fetchStatus();
    const interval = setInterval(fetchStatus, 30000); // каждые 30 секунд
  ...
  if (error) {
    return (
      <div className="max-w-[600px] mx-auto py-10 px-6 text-center space-y-3">
        ...
        <button onClick={() => router.push('/apply')} className="text-xs text-gray-400 hover:text-gray-600">
  ...
  return (
    <div className="max-w-[600px] mx-auto py-8 px-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-widest text-gray-400 font-semibold mb-0.5">
            inVision University
          </p>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 bg-white">
        <div
          ...
          style={{
            background:
              isDecided && c.decision === 'accepted'
                ? 'linear-gradient(90deg, #b5e220, #d9f99d)'
                : isDecided && c.decision === 'rejected'
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {c.submitted_at && (
                <p className="text-[11px] text-gray-300 mt-1">Submitted {formatDate(c.submitted_at)}</p>
              )}
            </div>
